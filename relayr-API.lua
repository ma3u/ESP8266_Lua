local M  = {
  _VERSION = '0.1',
  _NAME = 'relayrAPI',
  _DESCRIPTION = 'relayr API wrapper for the NodeMCU MQTT module',
  _DEBUG = false,
  _PORT = 1883,
...
local function subscribe_topics()
  ...
  client:subscribe({ [format('%s/cmd/', conf.topic)] = 0,
      [format('%s/config/', conf.topic)] = 0 },
    function(conn)
      -- Callback message on successful subscription.
      if M._DEBUG then print('Succesfully subscribed to "cmd" and "config" topics.') end
  end)
end
...
function M.send(data)
  ...
  if ok then
    client:publish(format('%s/data/', conf.topic), json, 0, 0,
                   function(client)
